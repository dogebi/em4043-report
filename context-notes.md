...
- 章节导航视觉调整采用“垂直基准线 + 横向章节线”设计，复用现有 section-dot 类名与滚动状态逻辑，避免改动导航行为。
- 章节导航改为按各报告章节实际文档位置比例布置线条，窗口尺寸变化或视频元数据加载后重新计算，以保持标题与导航间距同步。
- 本次补充在整体架构与逻辑标题下使用与知识库数据流一致的卡片和工作流列表，分别说明业务逻辑架构的 Agent 执行链路与组件架构的职责分工。
